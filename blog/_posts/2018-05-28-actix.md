...
back in 2010. You can think about it as an interesting mix of high-level
scripting languages like JavaScript or Ruby and a low-level high-performance
language like C++.
...
similar to `npm` in JavaScript or `pip` in Python, with the main difference
...
language and its descendant, [Elixir].
...
The complexity of this actor is relatively low, and that is because it is
usually hidden in the actor frameworks that are used to run these types of
...
While building the actor was relatively okay, running it is unfortunately still
...
The `Arbiter::start()` call returns us an `Addr`, that we can use to talk to
the actor. The `Addr` struct has methods like `send()` that can be used to send
messages to the actor and receive their responses.

As mentioned earlier, Rust is very strict around data ownership. Since we use
the `counter` variable for the first `send()` call, we are (at least currently)
not allowed to reuse it inside the callback. Instead we need to create a
`clone()` and use that one instead.
