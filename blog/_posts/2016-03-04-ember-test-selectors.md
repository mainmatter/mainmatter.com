...
layout: article
section: Blog
title: Use better element selectors in Ember.js tests
...
* custom test helpers that find elements by data attributes so that you don't have to write the quite long selectors yourselves; probably sth. like `findViaTestSelector('selector', 'post-title')`, https://github.com/simplabs/ember-test-selectors/issues/8
* template helpers that generate data attributes for elements, e.g. {% raw %}`<h1 {{test-selector="post-title"}}>{{post.title}}</h1>`{% endraw %} which would result in {% raw %}`<h1 data-test-selector="post-title">{{post.title}}</h1>`{% endraw %} or {% raw %}`<div {{test-selector post}}>…</div>`{% endraw %} which would result in `<div data-test-selector="post-1">…</div>`, https://github.com/simplabs/ember-test-selectors/issues/9
