...
module.exports = class ComponentsBuilder extends BaseComponentsBuilder {
  ...
  writeComponentFiles() {
    ...
    for (let workshop of this.workshops) {
      this._writeWorkshopPageComponentFile(workshop);
      this._writeWorkshopCardComponentFile(workshop);
    }
  }
  ...
  _prepareVideoComponentTemplateData(video) {
    ...
    let content = htmlizeMarkdown(video.content);
    let [body, cta] = splitContent(content);
    ...
    return {
      ...
      label: video.meta.label,
      title: video.meta.title,
      pageTitle: video.meta.pageTitle,
      moreText: video.meta.moreText,
      image: video.meta.image,
      teaser: video.meta.teaser,
  ...
  _writeWorkshopPageComponentFile(workshop) {
    let { componentName } = workshop;
    let data = this._prepareWorkshopPageComponentTemplateData(workshop);
    data = {
      ...data,
      siteUrl: SITE_URL,
    };
  ...
  _prepareWorkshopPageComponentTemplateData(workshop) {
  ...
  _writeWorkshopCardComponentFile(workshop) {
    let { cardComponentName: componentName } = workshop;
    let data = this._prepareWorkshopCardComponentTemplateData(workshop);
    data = {
      ...data,
    };
    let componentTemplate = this.templates['workshop-card'].template(data);

    data = {
      componentName,
    };
    let componentCssBlock = this.templates['workshop-card'].stylesheet(data);
    this.writeComponent(componentName, componentTemplate, componentCssBlock);
  }

  _prepareWorkshopCardComponentTemplateData(workshop) {
    return {
