  ...
  _writePageComponentTree(events) {
    ...
    let futureEvents = sortClosestFirst(filterFutureEventsOnly(events)).map((event) => {
      return {
        ...
        content: htmlizeBody(event.content),
      };
    });
    ...
    trees.push(writeFile('/src/ui/components/Calendar/template.hbs', componentTemplate));

    let componentCssBlock = this._templates['css-block']();
    trees.push(writeFile('/src/ui/components/Calendar/stylesheet.css', componentCssBlock));
