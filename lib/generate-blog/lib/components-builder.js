...
module.exports = class ComponentsBuilder extends BaseComponentsBuilder {
  ...
  _writePostComponent(post, related) {
    ...
    let componentImplementation = this.templates.post.component(data);

    this.writeComponent(post.componentName, componentTemplate, componentCssBlock, componentImplementation);
  }
...
function htmlizeBody(content) {
  let html = htmlizeMarkdown(content, { renderer: Renderer }, dom => {
    let replaceWithFigure = (element, content) => {
    ...
    dom.querySelectorAll('p > img').forEach(img => {
      ...
      replaceWithFigure(paragraph, img);
    });
    ...
    dom.querySelectorAll('p > a > img').forEach(img => {
      ...
      replaceWithFigure(paragraph, img.parentElement);
    });
  });
