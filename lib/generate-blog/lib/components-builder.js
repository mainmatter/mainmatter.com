...
module.exports = class ComponentsBuilder extends BaseComponentsBuilder {
  ...
  writeComponentFiles() {
    ...
    chunks.forEach((chunk, i) => this._writeListPageComponent(chunk, i + 1));
  ...
  _writeListPageComponent(chunk, page) {
    ...
    data.posts = posts.map(post => this._preparePostTemplateData(post));
