...
module.exports = class ComponentsBuilder extends BaseComponentsBuilder {
  ...
  _writeListPageComponent(chunk, page, totalPages) {
    ...
    if (page === 1) {
      ...
      posts = rest;
      data.latest = this._preparePostTemplateData(latest, true);
    }
  ...
  _writePostComponent(post, related) {
    let data = this._preparePostTemplateData(post, true);
  ...
  _preparePostTemplateData(post, isExcerptLeadText = false) {
    assert(
    ...
    return {
      ...
      excerpt: htmlizeExcerpt(post.content, isExcerptLeadText),
...
function htmlizeExcerpt(content, isLeadText) {
  let html = htmlizeMarkdown(content, (dom) => {
    styleBlockQuotes(dom, isLeadText);
  });
