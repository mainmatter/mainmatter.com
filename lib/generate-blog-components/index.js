...
function prepareTemplates() {
  let components = ['post', 'author', 'start-page'];
  return components.reduce((acc, component) => {
    let files = glob.sync('*.hbs', {
      cwd: path.join(__dirname, 'lib', 'files', component),
      absolute: true,
    });
    files.forEach(file => {
      let source = fs.readFileSync(file).toString();
      let prop = path.basename(file, '.hbs');
      acc[component] = {
        ...(acc[component] || {}),
        [prop]: handlebars.compile(source),
      };
    });
