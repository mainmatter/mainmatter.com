...
function htmlizeBody(body) {
  ...
  return html
    .replace(/<h2([^>]*)>/gm, '<h2$1 class="typography.sub-heading">')
    .replace(/<p([^>]*)>/gm, '<p$1 class="typography.body-text">')
    .replace(/<p([^>]*)>\s*<img([^>]*)>\s*<\/p>/gm, '<div class="main-block.body-image"><img$2 class="main-block.image-centered"></div>');
}

function htmlizeExcerpt(excerpt) {
  let html = encodeCurlies(marked(excerpt, { renderer: Renderer }));
  return html.trim().replace(/<p>/gm, '<p class="typography.lead">');
}
