...
function htmlizeBody(body) {
  return encodeCurlies(marked(body, { renderer: Renderer }));
}

function htmlizeExcerpt(excerpt) {
  let htmlized = encodeCurlies(marked(excerpt, { renderer: Renderer }));
  return htmlized.trim().replace(/<p>/g, '<p class="typography.lead">');
}
