  ...
  preprocessTree(type, tree) {
    if (type === 'src') {
      ...
      let blogPostTrees = posts.map(post => {
        let htmlized = encodeCurlies(marked(post.content, { renderer }));
        return mergeTrees([
          writeTemplate(post.componentName, htmlized, post.meta),
...
function writeTemplate(componentName, content, meta) {
  return writeFile(
